...
export function LessonCoveragePanel({ document, plan }: { document: LessonDocument; plan: LessonPlan }) {
  ...
  return <details className="lesson-coverage-panel">
    ...
    {!exceptions.length && !ranges.length && !plan.topic?.exclusions.length ? <p>No compression or omissions are declared in the approved plan.</p> : <ul>
      {plan.topic?.exclusions.map(item => <li key={item}>{item}</li>)}
    ...
    <h3>{plan.topic ? 'Where the objectives are taught' : 'Where the source is taught'}</h3>
    {document.coverage_review?.length ? <ol className="coverage-concepts">{document.coverage_review.map((entry, index) => <li key={index}>
      ...
      {entry.source_element_ids.length ? <small>Evidence: {entry.source_element_ids.join(', ')}</small> : null}
