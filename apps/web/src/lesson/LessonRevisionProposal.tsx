...
export function LessonRevisionProposal({ document, onError, onOpenEvidence, topic, onOpenTopicEvidence }: { topic?: TopicLessonScope; onOpenTopicEvidence?: (sourceId: string, elementId: string, returnTargetId?: string) => Promise<void>; document: LessonDocument; onError: (message: string) => void; onOpenEvidence: (elementId: string, returnTargetId?: string) => Promise<void> }) {
  ...
  return <section className="lesson-edit-proposal" aria-label="Proposed lesson revision">
    ...
    {changes.map(({ id, previous, proposed, moved, changed }, index) => {
      ...
      return <details className="revision-change" key={id} open={changes.length === 1}><summary>{label ?? `Change ${index + 1}`} · {old ? next ? changed ? 'Revised' : 'Moved' : 'Removed' : 'Added'}</summary>
        ...
        <div className="revision-comparison">{[{ label: 'Current', block: old }, { label: 'Proposed', block: next }].map(({ label: side, block }) => <div key={side}><strong>{side}</strong><RevisionContent content={block?.content} sourceId={topic ? document.plan_id : document.source_id} />{block && topic ? <><TopicReferences references={topic.references} ids={block.reference_ids ?? []} onOpenEvidence={onOpenTopicEvidence} onError={onError} /><p className="revision-note">Objectives: {block.objective_ids?.join(", ")}</p></> : null}{block ? <><p className="revision-note">{block.provenance.replaceAll('_', ' ')}</p><div className="revision-evidence">{block.source_element_ids.map((elementId, i) => { const target = `revision-evidence-${index}-${side}-${i}`; return <button type="button" className="quiet-button" id={target} key={elementId} onClick={() => setEvidence({ elementId, referenceIds: block.source_element_ids, returnTargetId: target })}>Evidence {i + 1}</button> })}</div></> : null}</div>)}</div>
        {JSON.stringify([old?.source_element_ids, old?.reference_ids, old?.objective_ids]) !== JSON.stringify([next?.source_element_ids, next?.reference_ids, next?.objective_ids]) ? <p className="revision-note">References or objective mappings changed. Inspect either version’s evidence before accepting.</p> : null}
      </details>
    })}
...
function RevisionContent({ content, sourceId }: { content?: LessonBlockContent; sourceId: string }) {
  ...
  if (content.kind === 'practice') return <aside className="lesson-practice"><h4>{content.prompt}</h4>{content.hints.length ? <><strong>Hints</strong><ul>{content.hints.map(hint => <li key={hint}>{hint}</li>)}</ul></> : null}<strong>Worked response</strong><p>{content.solution}</p><p><strong>Reflect:</strong> {content.reflection}</p></aside>
