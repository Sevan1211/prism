...
describe('PRISM source workspace', () => {
  ...
  it('imports through the browser-local source dialog instead of a hosted upload path', async () => {
    ...
    expect(setBrowserAgentContentAccess).toHaveBeenCalledWith(imported.id, true)
  })

  it.each(['private_authorized', 'unknown'] as const)('automatically grants %s source access on upload and allows revocation', async (rights) => {
    ...
    expect(screen.getByText(/Adding it enables your connected agent/)).toBeVisible()
    ...
    await waitFor(() => expect(setBrowserAgentContentAccess).toHaveBeenCalledWith(imported.id, false))
  })

  it('does not grant access before an upload succeeds or when import is cancelled', async () => {
    render(<App />)
    fireEvent.click(await screen.findByRole('button', { name: 'Add PDF' }))
