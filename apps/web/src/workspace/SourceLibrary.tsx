...
export function SourceLibrary({ sources, sourcesReady, onImport, children, selectedSource }: {
  ...
  const [topicSeries, setTopicSeries] = useState<TopicSeries[]>([])
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        const [next, topics] = await Promise.all([loadSourceFolders(), listTopicSeries()])
        if (active && request === sequence) { setLibrary(next); setTopicSeries(topics); setFoldersReady(true) }
      } catch (cause) {
    ...
    const unsubscribe = subscribeSourcesChanged(onChange)
    window.addEventListener(PRISM_VAULT_CHANGED_EVENT, onChange)
    return () => { active = false; unsubscribe(); window.removeEventListener(PRISM_VAULT_CHANGED_EVENT, onChange) }
  }, [])
  ...
  const countIn = (id: string) => sources.filter(source => (assignments.get(source.id) ?? 'unfiled') === id).length + topicSeries.filter(series => (series.folder_id && folderIds.has(series.folder_id) ? series.folder_id : 'unfiled') === id).length
  const title = folder?.name ?? (selection === 'unfiled' ? 'Unfiled' : 'All learning')
  ...
  return (
    <div className="source-library">
      {pendingDelete ? <ConfirmationDialog
        ...
        <p>{countIn(pendingDelete.id) ? `${countIn(pendingDelete.id)} library items move to Unfiled.` : 'This folder is empty.'} Your PDFs, lessons, and reading history are kept.</p>
        {synced.connected ? <p className="confirmation-note">The folder removal also syncs to your connected browsers.</p> : null}
      </ConfirmationDialog> : null}
      <AppHeader libraryActive={!children} count={sources.length + topicSeries.length} onImport={() => onImport(folder?.id ?? null)} />
      <div className="library-layout">
        <aside className="folder-sidebar" aria-label="Library folders">
          <nav aria-label="Browse library"><button type="button" aria-current={selection === 'all' ? 'page' : undefined} onClick={() => chooseFolder('all')}><Stack aria-hidden="true" /><span>Library</span><small>{sources.length + topicSeries.length}</small></button><button type="button" aria-current={selection === 'unfiled' ? 'page' : undefined} onClick={() => chooseFolder('unfiled')}><Tray aria-hidden="true" /><span>Unfiled</span><small>{countIn('unfiled')}</small></button></nav>
        ...
        <main className="library-content" id="workspace-main" tabIndex={-1}>
          ...
          <div className="source-library-toolbar"><label className="library-search"><MagnifyingGlass aria-hidden="true" /><span className="sr-only">Search your library</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder={folder ? 'Search this folder…' : 'Search your library…'} /></label><label className="library-sort"><span className="sr-only">Sort sources</span><select value={sort} onChange={event => setSort(event.target.value)}><option value="recent">Recently added</option><option value="title">Title A–Z</option></select></label></div>
          <TopicLibrary key={selection} folderId={selection === 'all' ? 'all' : selection === 'unfiled' ? null : selection} query={query.trim()} sources={sources} />
          <h3 className="topic-sources-heading">Sources <span>Original material for reading and reference</span></h3>
          ...
          {sourcesReady && filtered.length === 0 ? <div className="library-blank"><Books weight="duotone" aria-hidden="true" /><h3>{query ? 'No matching sources' : folder ? 'Add supporting material.' : selection === 'unfiled' && sources.length ? 'Everything has a place.' : 'Your sources belong here.'}</h3><p>{query ? 'Try a different title, or clear your search.' : folder ? 'Add a PDF here, or move a source into this folder from the Library.' : selection === 'unfiled' && sources.length ? 'All your sources are organized into folders.' : 'Add a PDF to read the original or create lessons from its contents. Topic lessons do not require an upload.'}</p>{query ? <button className="button-secondary" type="button" onClick={() => setQuery('')}>Clear search</button> : <button className="button-secondary" type="button" onClick={() => onImport(folder?.id ?? null)}><Plus aria-hidden="true" />Choose your PDF</button>}</div> : null}
